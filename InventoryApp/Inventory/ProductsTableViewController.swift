...
class ProductsTableViewController: UITableViewController, UIPopoverPresentationControllerDelegate, UIGestureRecognizerDelegate, UISearchBarDelegate, SortOptionsSelectionProtocol {
    ...
    override func viewDidLoad() {
        ...
        products = realm.objects(Product.self).sorted(byKeyPath: sortProperty, ascending: sortAscending)
    ...
    func restortEntries() {
        ...
        products = self.products?.sorted(byKeyPath: self.sortProperty, ascending: self.sortAscending)
        tableView.reloadData()
    }
    ...
    func findByProductID(productId: String) -> Bool{
        return products?.filter("id = %@", productId) != nil
    }
